...
namespace PaintServer.DAL
{
...

    {
        ...
        public SaveImageResultData SaveImage(string name, int size, string imageType, int userId, DateTime dateTime)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                var queryString = $"INSERT INTO dbo.SavedImages (UserId, ImageName, CreateDate, FileSize,ImageType) VALUES ('{userId}','{name}','{dateTime}','{size}','{imageType}')";
                connection.Open();
